$ErrorActionPreference = "Stop"

function Build-App ($type, $name) {
    Write-Host "----------------------------------------------------------------"
    Write-Host "BUILDING: $name ($type)" -ForegroundColor Cyan
    Write-Host "----------------------------------------------------------------"

    $env:APP_TYPE = $type
    
    # 1. Expo Prebuild (Critical for switching package name/icon)
    Write-Host "Running expo prebuild for $type..."
    cmd /c "npx expo prebuild --clean --platform android --no-install"
    if ($LASTEXITCODE -ne 0) { Write-Error "Prebuild failed for $type"; exit 1 }

    # 2. Local Properties
    $sdkPath = "$env:LOCALAPPDATA\Android\Sdk"
    $localProps = "android\local.properties"
    if (Test-Path $sdkPath) {
        $sdkPathEscaped = $sdkPath -replace "\\", "\\"
        "sdk.dir=$sdkPathEscaped" | Out-File -FilePath $localProps -Encoding ascii
    }

    # 3. Mount X: Drive
    if (Test-Path "X:\") { cmd /c "subst X: /D" }
    cmd /c "subst X: $PSScriptRoot"
    if (-not (Test-Path "X:\")) { Write-Error "Failed to mount X: drive."; exit 1 }

    try {
        Push-Location "X:\android"
        
        # Clean Gradle
        Write-Host "Cleaning valid gradle cache..."
        cmd /c "gradlew --stop"
        if (Test-Path ".gradle") { Remove-Item ".gradle" -Recurse -Force -ErrorAction SilentlyContinue }
        
        # Build
        Write-Host "Assembling Release APK..."
        $tempCache = "$env:TEMP\marwad-gradle-cache"
        if (-not (Test-Path $tempCache)) { New-Item -ItemType Directory -Path $tempCache -Force | Out-Null }
        
        # Redirect output to file for debugging
        cmd /c "gradlew assembleRelease --no-daemon --project-cache-dir `"$tempCache`" > ..\build_$type.log 2>&1"
        
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "Gradle build exited with error code. Checking log..."
        }
    }
    finally {
        Pop-Location
        cmd /c "subst X: /D"
    }

    # 4. Verify and Copy
    $outputDir = "android\app\build\outputs\apk\release"
    # Also check the direct path in case we are still in X module
    if (Test-Path "$outputDir\app-release.apk") {
        if (-not (Test-Path "..\APKs")) { New-Item -ItemType Directory -Path "..\APKs" -Force | Out-Null }
        $dest = "..\APKs\Marwad-$name-Fresh.apk"
        Copy-Item "$outputDir\app-release.apk" -Destination $dest -Force
        Write-Host "SUCCESS! $name APK created at: $dest" -ForegroundColor Green
    }
    elseif (Test-Path "X:\android\app\build\outputs\apk\release\app-release.apk") {
        if (-not (Test-Path "..\APKs")) { New-Item -ItemType Directory -Path "..\APKs" -Force | Out-Null }
        $dest = "..\APKs\Marwad-$name-Fresh.apk"
        Copy-Item "X:\android\app\build\outputs\apk\release\app-release.apk" -Destination $dest -Force
        Write-Host "SUCCESS! $name APK created at: $dest" -ForegroundColor Green
    }
    else {
        Write-Error "$name APK build completed but file not found. Check build_$type.log."
    }
}

# Build Admin
Build-App "admin" "Admin"

# Build Staff
Build-App "staff" "Staff"
